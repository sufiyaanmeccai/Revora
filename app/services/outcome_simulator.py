...
async def simulate_active_interventions(db: AsyncSession) -> Dict[str, int]:
    ...
    for event in events:
        ...
        if roll < SUCCESS_PROBABILITY:
            # ── SUCCESS PATH (65%) — via Unified Reconciliation Service ───────
            recon_res = await reconcile_payment_success(
            ...
            counts["recovered"] += 1
            rec_amt = recon_res.get("amount_recovered", event.amount) if recon_res else event.amount

            logger.info(
                "Outcome simulator: event %s → RECOVERED (₹%.2f captured, strategy=%s) via reconciliation.",
                event.id,
                rec_amt,
                wf_strat,
            )
